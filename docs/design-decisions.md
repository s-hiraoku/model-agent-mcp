...
| 「fallback機能を設けて、CLIProxyAPIのデフォルトを使う」 | CLIProxyAPI にモデル省略時の共通既定値は確認できず、利用可能モデルを選ぶ `auto` selector を一回だけ使う |
| 「まず使う時に、Model Agent MCPをmodel ○○で使います。と絶対表示」 | MCP instructions と利用側の AGENTS.md 例に表示指示を入れる。表示は呼び出し元エージェントが行う |
